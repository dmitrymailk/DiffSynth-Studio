...
class QwenImageBlockwiseMultiControlNet(torch.nn.Module):
    def __init__(self, models: list[QwenImageBlockWiseControlNet]):
        ...
        for model in models:
            if hasattr(model, "vram_management_enabled") and getattr(
                model, "vram_management_enabled"
            ):
        ...
        **kwargs,
        ...
        for controlnet_input, conditioning in zip(controlnet_inputs, conditionings):
            conditioning = rearrange(
                conditioning, "B C (H P) (W Q) -> B (H W) (C P Q)", P=2, Q=2
            )
            model_output = self.models[
                controlnet_input.controlnet_id
    ...
    def blockwise_forward(
        ...
        **kwargs,
        ...
        for controlnet_input, conditioning in zip(controlnet_inputs, conditionings):
            ...
            if progress > controlnet_input.start + (
                1e-4
            ) or progress < controlnet_input.end - (1e-4):
                continue
            model_output = self.models[
                controlnet_input.controlnet_id
            ].blockwise_forward(image, conditioning, block_id)
...
class QwenImageUnit_NoiseInitializer(PipelineUnit):
    ...
    def process(
        self, pipe: QwenImagePipeline, height, width, seed, rand_device, layer_num
    ):
        if layer_num is None:
...
class QwenImageUnit_InputImageEmbedder(PipelineUnit):
    ...
    def process(
        self, pipe: QwenImagePipeline, input_image, noise, tiled, tile_size, tile_stride
    ):
...
class QwenImageUnit_LayerInputImageEmbedder(PipelineUnit):
    ...
    def process(
        self, pipe: QwenImagePipeline, layer_input_image, tiled, tile_size, tile_stride
    ):
        if layer_input_image is None:
            return {}
        pipe.load_models_to_device(["vae"])
        image = pipe.preprocess_image(layer_input_image).to(
            device=pipe.device, dtype=pipe.torch_dtype
        )
...
class QwenImageUnit_Inpaint(PipelineUnit):
    ...
    def process(
        ...
        if inpaint_blur_size is not None and inpaint_blur_sigma is not None:
            ...
            blur = GaussianBlur(
                kernel_size=inpaint_blur_size * 2 + 1, sigma=inpaint_blur_sigma
            )
...
class QwenImageUnit_PromptEmbedder(PipelineUnit):
    ...
    def resize_image(self, image, target_area=384 * 384):
        width, height = self.calculate_dimensions(
            target_area, image.size[0] / image.size[1]
        )
    ...
    def encode_prompt(self, pipe: QwenImagePipeline, prompt):
        ...
        split_hidden_states = self.extract_masked_hidden(
    ...
    def encode_prompt_edit(self, pipe: QwenImagePipeline, prompt, edit_image):
        ...
        model_inputs = pipe.processor(
            text=txt, images=edit_image, padding=True, return_tensors="pt"
        ).to(pipe.device)
        hidden_states = pipe.text_encoder(
            input_ids=model_inputs.input_ids,
            attention_mask=model_inputs.attention_mask,
    ...
    def process(self, pipe: QwenImagePipeline, prompt, edit_image=None) -> dict:
        ...
        if pipe.text_encoder is not None:
            ...
            if edit_image is None:
                ...
                split_hidden_states = self.encode_prompt_edit_multi(
                    pipe, prompt, edit_image
                )
            attn_mask_list = [
...
class QwenImageUnit_EntityControl(PipelineUnit):
    ...
    def prepare_eligen(
        ...
        entity_prompt_emb_posi, entity_prompt_emb_posi_mask, entity_masks_posi = (
            self.prepare_entity_inputs(
                pipe, eligen_entity_prompts, eligen_entity_masks, width, height
            )
        )
        if enable_eligen_on_negative and cfg_scale != 1.0:
            entity_prompt_emb_nega = [prompt_emb_nega["prompt_emb"]] * len(
                entity_prompt_emb_posi
            )
            entity_prompt_emb_nega_mask = [prompt_emb_nega["prompt_emb_mask"]] * len(
                entity_prompt_emb_posi
            )
            entity_masks_nega = entity_masks_posi
        else:
            entity_prompt_emb_nega, entity_prompt_emb_nega_mask, entity_masks_nega = (
                None,
                None,
                None,
            )
    ...
    def process(self, pipe: QwenImagePipeline, inputs_shared, inputs_posi, inputs_nega):
        ...
        pipe.load_models_to_device(self.onload_model_names)
        eligen_enable_on_negative = inputs_shared.get(
...
class QwenImageUnit_EditImageEmbedder(PipelineUnit):
    ...
    def process(
        ...
        if isinstance(edit_image, Image.Image):
            resized_edit_image = (
                self.edit_image_auto_resize(edit_image)
                if edit_image_auto_resize
                else edit_image
            )
            edit_image = pipe.preprocess_image(resized_edit_image).to(
                device=pipe.device, dtype=pipe.torch_dtype
            )
            edit_latents = pipe.vae.encode(
                edit_image, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride
            )
        else:
...
class QwenImageUnit_Image2LoRAEncode(PipelineUnit):
    ...
    def encode_images_using_siglip2(
    ...
    def encode_images_using_dinov3(
        self, pipe: QwenImagePipeline, images: list[Image.Image]
    ):
    ...
    def encode_images_using_qwenvl(
        ...
        for image in images:
            image = (
                self.processor_highres(image)
                if highres
                else self.processor_lowres(image)
            )
    ...
    def encode_images(self, pipe: QwenImagePipeline, images: list[Image.Image]):
        ...
        if pipe.image2lora_fine is not None:
            residual_highres = self.encode_images_using_qwenvl(
                pipe, images, highres=True
            )
    ...
    def process(self, pipe: QwenImagePipeline, image2lora_images):
        ...
        return {
            "image2lora_x": x,
            "image2lora_residual": residual,
            "image2lora_residual_highres": residual_highres,
        }
...
class QwenImageUnit_ContextImageEmbedder(PipelineUnit):
    ...
    def process(
        ...
        context_latents = pipe.vae.encode(
            context_image, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride
        )
...
def model_fn_qwen_image(
    ...
    **kwargs,
    ...
    if edit_latents is not None:
        edit_latents_list = (
            edit_latents if isinstance(edit_latents, list) else [edit_latents]
        )
        img_shapes += [
            (e.shape[0], e.shape[2] // 2, e.shape[3] // 2) for e in edit_latents_list
        ]
        edit_image = [
            rearrange(
                e,
    ...
    if layer_input_latents is not None:
        ...
        layer_input_latents = rearrange(
            layer_input_latents, "B C (H P) (W Q) -> B (H W) (C P Q)", P=2, Q=2
        )
    ...
    if entity_prompt_emb is not None:
        ...
        if edit_rope_interpolation:
            image_rotary_emb = dit.pos_embed.forward_sampling(
                img_shapes, txt_seq_lens, device=latents.device
            )
        else:
            image_rotary_emb = dit.pos_embed(
                img_shapes, txt_seq_lens, device=latents.device
            )
        attention_mask = None
