...
def DirectDistillLoss(pipe: BasePipeline, **inputs):
    ...
    for progress_id, timestep in enumerate(pipe.scheduler.timesteps):
        ...
        noise_pred = pipe.model_fn(
            **models, **inputs, timestep=timestep, progress_id=progress_id
        )
...
class TrajectoryImitationLoss(torch.nn.Module):
    ...
    def fetch_trajectory(
        ...
        pipe.scheduler.set_timesteps(
            num_inference_steps, target_timesteps=timesteps_student
        )
        models = {name: getattr(pipe, name) for name in pipe.in_iteration_models}
        for progress_id, timestep in enumerate(pipe.scheduler.timesteps):
    ...
    def align_trajectory(
        ...
        for progress_id, timestep in enumerate(pipe.scheduler.timesteps):
            ...
            sigma_ = (
                ...
                else pipe.scheduler.sigmas[progress_id + 1]
            )
            if progress_id + 1 >= len(pipe.scheduler.timesteps):
                ...
                progress_id_teacher = torch.argmin(
                    (
                        timesteps_teacher - pipe.scheduler.timesteps[progress_id + 1]
                    ).abs()
                )
    ...
    def forward(self, pipe: BasePipeline, inputs_shared, inputs_posi, inputs_nega):
        ...
        with torch.no_grad():
            ...
            timesteps_teacher = timesteps_teacher.to(
                dtype=pipe.torch_dtype, device=pipe.device
            )
        ...
        loss_2 = self.compute_regularization(
            pipe, trajectory_teacher, inputs_shared, inputs_posi, inputs_nega, 8, 1
        )
        loss = loss_1 + loss_2
        return loss
